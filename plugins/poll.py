...
class PollPlugin(Plugin):
    ...
    def process_message(self, data):
        ...
        if arg[0] != PREFIX + 'poll':
            return
        if len(arg) == 1: # TODO help
            self._say(data, 'not enough args')
            return
        cmd = arg[1]
        if cmd == 'create':
        ...
        elif cmd in self.polls:
            # !poll <pollid> - shows poll status
            res = self.poll_get_results(cmd)
            opt = self.poll_get_options(cmd)
